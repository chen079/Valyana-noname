...
export default {
	...
	async content(event, trigger, player) {
		if (!player.hasStorage('vl_wes_ts')) player.setStorage('vl_wes_ts', []);
		game.findPlayer2(function (current) {
			if (player.getStorage('vl_wes_ts', []).includes(current)) {
				player.getStorage('vl_wes_ts', []).remove(current);
		...
		if (result.bool) {
			...
			player.line(target);
			player.markAuto('vl_wes_ts', target);
		...
		a: {
			...
			filter(event, player) {
				...
				return game.countPlayer(function (current) {
					if (current.getStorage('vl_wes_ts', []).includes(player) && event.player != current) return true;
				});
				return false;
			},
			async content(event, trigger, player) {
				game.countPlayer(function (current) {
					if (current.getStorage('vl_wes_ts', []).includes(player)) {
			...
			onremove(player) {
				game.findPlayer2(function (current) {
					if (current.getStorage('vl_wes_ts', []).includes(player)) {
						current.getStorage('vl_wes_ts', []).remove(player);
						if (!current.getStorage('vl_wes_ts', []).length) current.unmarkSkill('vl_wes_ts');
