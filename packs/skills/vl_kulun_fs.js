...
export default {
    ...
    async dieContent(event, trigger, player) {
					const { source } = event;
					event.forceDie = true;
					if (source) {
...
trigger._vl_kulun_fs_return = true;
        							...
        							const recoverEvent = trigger.player.recover(trigger.player.maxHp - trigger.player.hp);
        							game.broadcastAll(function (player) {
...
await event.trigger('restEnd');
    ...
    t: {
        name: "反噬",
        info: `锁定技。①当你死亡前，若你有未注入过的元素且你的体力上限大于0，你改为${get.poptip("xiuzheng")}。②回合结束后，你死亡。`,
    },
};
